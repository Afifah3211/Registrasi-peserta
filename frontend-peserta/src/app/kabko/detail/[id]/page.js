...
export default async function DetailKabkoPage({ params }) {
    ...
    return (
      <div className="mx-auto max-w-4xl p-6">
        <div className="rounded-2xl bg-red-50 p-6 text-red-600 ring-1 ring-red-200">
          ...
          <p>{error}</p>
          <Link href="/kabko" className="mt-4 inline-block text-slate-600 hover:text-slate-900">
            &larr; Kembali ke Daftar Kabupaten/Kota
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-slate-900">Detail Kabupaten/Kota</h1>
        <Link
          href="/kabko"
      ...
      <div className="overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <dl>
          <LabelValue label="ID Kab/Kota" value={kabko.id} />
          <LabelValue label="Nama Kab/Kota" value={kabko.nama} />
          <LabelValue label="ID Provinsi" value={kabko.id_provinsi} />
          <LabelValue label="Nama Provinsi" value={kabko.nama_provinsi} />
        </dl>
      </div>
      
      <div className="mt-6 flex justify-end gap-3">
        <Link
          href={`/kabko/edit/${kabko.id}`}
